...
-- @usage global_equality(child, "_G.WorkspaceDiagnosticsLoaded", true)
...
-- @usage global_type_equality(child, "_G.WorkspaceDiagnosticsLoaded", "boolean")
...
Helpers.expect.config_equality = MiniTest.new_expectation(
  ...
  function(child, field, value)
    return Helpers.expect.global_equality(child, "_G.WorkspaceDiagnostics.config." .. field, value)
  end,
  errorMessage
)
...
Helpers.expect.config_type_equality = MiniTest.new_expectation(
  ...
  function(child, field, value)
    return Helpers.expect.global_equality(
      child,
      "type(_G.WorkspaceDiagnostics.config." .. field .. ")",
      value
    )
  end,
  errorMessage
...
Helpers.expect.state_equality = MiniTest.new_expectation(
  "state matches",
  function(child, field, value)
    return Helpers.expect.global_equality(child, "_G.WorkspaceDiagnostics.enabled." .. field, value)
  end,
  errorMessage
)
...
Helpers.expect.state_type_equality = MiniTest.new_expectation(
  ...
  function(child, field, value)
    return Helpers.expect.global_equality(
      child,
      "type(_G.WorkspaceDiagnostics.state." .. field .. ")",
      value
    )
  end,
  errorMessage
